...
//	  @CircuitBreaker(name="${spring.application.name}", fallbackMethod ="getDefaultDepartment" )
	  @Retry(name="${spring.application.name}",fallbackMethod="getDefaultDepartment") //instead of Circuitbreaker method we are using Retry to method to automatically call this method to check if it is just temporarily down .
	  public APIResponseDto getEmployeeById(Long employeeId) {
		  LOGGER.info("inside getEmployeeById() method");
		  Employee employee = employeeRepository.findById(employeeId).get();
		  
//		  ResponseEntity<DepartmentDto> responseEntity =  restTemplate.getForEntity("http://localhost:8080/api/departments/"+employee.getDepartmentCode(),DepartmentDto.class );
		 //This code is using the RestTemplate class to send an HTTP GET request to the specified URL 
		  //and retrieve the response as a DepartmentDto object
